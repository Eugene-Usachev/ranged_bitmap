...
pub const BITS_IN_USIZE: usize = usize::BITS as usize;
...
pub const fn blocks_number_for_bits(requested_bits: usize) -> usize {
    (requested_bits / BITS_IN_USIZE)
        + if requested_bits % BITS_IN_USIZE != 0 {
            1
        } else {
            0
        }
...
#[inline(always)]
pub const fn calc_block_and_slot(bit: usize) -> (usize, usize) {
    (bit / BITS_IN_USIZE, bit % BITS_IN_USIZE)
}
...
pub const fn bitmap_set(blocks: &mut [usize], bit: usize) {
    maybe_assert(bit < blocks.len() * BITS_IN_USIZE, "`bit` is out of bounds");
...
const fn update_range(
    ...
    start: usize,
    len: usize,
    set_bits_for_len: &[usize; BITS_IN_USIZE + 1],
    is_set_operation: bool,
) {
    maybe_assert(
        start + len <= blocks.len() * BITS_IN_USIZE,
    ...
    if end <= BITS_IN_USIZE {
    ...
    } else if end <= BITS_IN_USIZE * 2 {
        // two blocks
        let first_block_len = BITS_IN_USIZE - start_bit;
        ...
        let number_of_full_blocks = remaining / BITS_IN_USIZE;
        let last_block_len = remaining % BITS_IN_USIZE;
        let first_mask = *get_ref_by_index(set_bits_for_len, first_block_len) << start_bit;
...
const fn check_range_internal(
    ...
    start: usize,
    len: usize,
    check_bits_for_len: &[usize; BITS_IN_USIZE + 1],
    is_set_check: bool,
) -> bool {
    maybe_assert(
        start + len <= blocks.len() * BITS_IN_USIZE,
    ...
    if end <= BITS_IN_USIZE {
        ...
        if last_block_len > 0 {
            ...
            ones += (*get_ref_by_index(blocks, block + number_of_full_blocks + 1) & second_mask)
                .count_ones();
        }
